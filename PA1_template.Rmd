...
Thus, the mean of the total number of steps taken each day is `r MeanStep` and the median of the total number of steps taken each day is `r MedianStep`.
...
Thus, the 5-minute interval, which contains the maximum number of steps, is `r MaxInt`.
...
Total number of missing values in the date set is `r Len`
...
The mean of total number of steps taken per day is `r MeanStep_new` and the median is `r MedianStep_new`.

Based on the results, we can find:

* For first part: Mean = `r MeanStep` and Median = `r MedianStep`

* For this part: Mean = `r MeanStep_new` and Median = `r MedianStep_new`
